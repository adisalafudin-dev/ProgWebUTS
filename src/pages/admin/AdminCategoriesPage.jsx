  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="category-chart-heading">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-indigo-600">Statistik</p>
          <h3 id="category-chart-heading" className="mt-1 font-playfair text-xl font-bold text-slate-900">5 subject paling populer</h3>
        </div>
        <Icon name="flame" className="h-5 w-5 text-amber-500" />
      </div>
      <div className="mt-5 space-y-3">
        {chartItems.map((category) => {
          const percentage = percentageOf(category.bookCount, totalBooks);
          return (
            <div key={category.id}>
              <div className="mb-1 flex items-center justify-between gap-3 text-xs">
                <span className="truncate font-semibold text-slate-700">{category.name}</span>
                <span className="shrink-0 font-bold text-slate-900">{category.bookCount} buku</span>
              </div>
              <div className="h-2 overflow-hidden rounded-full bg-slate-100" role="progressbar" aria-label={`${category.name}: ${percentage.toFixed(1)} persen`} aria-valuenow={percentage} aria-valuemin="0" aria-valuemax="100">
                <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-500" style={{ width: `${Math.max(percentage, 2)}%` }} />
              </div>
            </div>
          );
        })}
...
export default function AdminCategoriesPage() {
  ...
  const processedCategories = useMemo(() => {
    ...
    const result = keyword
      ? categories.filter((category) => category.name.toLocaleLowerCase().includes(keyword))
      : [...categories];
  ...
  const popularCategory = useMemo(
    () => [...categories].sort((first, second) => second.bookCount - first.bookCount)[0],
    [categories],
  );
  ...
  const openCategoryBooks = (subject) => {
    navigate(`/admin/books?subject=${encodeURIComponent(subject)}`);
  };
